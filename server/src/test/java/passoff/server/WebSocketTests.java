...
public class WebSocketTests {
    ...
    public void connectSingleUser() {    //Connects 1 User to the game
    ...
    public void connectBadGameID() {  //player and observer connect with an incorrect game id
    ...
    public void connectBadAuthToken() {
        connectToGame(new WebsocketUser("", "badAuth"), gameID, false, Set.of(), Set.of(), "connect with bad auth");
    }
    ...
    private void assertMessages(String username, ServerMessage.ServerMessageType[] expectedTypes, List<TestMessage> messages, String description) {
        Assertions.assertEquals(expectedTypes.length, messages.size(), "For command '%s' player '%s' expected %d messages, got %d: %s"
        ...
        try {
            ...
            Assertions.fail("\nFor command '%s' player '%s' expected message types matching %s\nGot: %s\nCause: %s"
